...
export class UserApiService {
  ...
  update(user: User): Observable<User> {
    return this.httpClient.post<User>('/users/update', user);
  }
  ...
  login(loginFormValue: any): Observable<any> {
	console.log("login");
    return this.httpClient.get<any>(`/login?username=${loginFormValue.username}&password=${loginFormValue.password}`, {});
  }
